...
def resolve_titan_root() -> Path:
    env_root = os.environ.get("OKASINA_ROOT")
    if env_root:
...
BRIDGE_API_PATH = TITAN_ROOT / "package.json"
...
def action_restart_service() -> dict:
    """Touches package.json to trigger Uvicorn reload"""
    print("[ACTION] Restarting Service (Touching package.json)...")
    try:
        if not BRIDGE_API_PATH.exists():
            return {"outcome": "FAILED", "notes": "package.json not found"}
...
def main():
    ...
    if not report:
        print("[WARN] No report found or empty.")
        # Create empty ledger log? No, just exit.
        return
    ...
    for sig in signals:
        ...
        print(f"\nProcessing Signal: {sig_type} (ID: {sig['signal_id']})")
